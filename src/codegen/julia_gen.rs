...
pub fn generate_julia_module(unit: &TranslationUnit, manifest_mappings: &[MappingEntry]) -> String {
    ...
    for mapping in manifest_mappings {
        for (src_fn, julia_fn) in &mapping.overrides {
            user_overrides.insert((mapping.from_lib.clone(), src_fn.clone()), julia_fn.clone());
        }
    }
    ...
    // Header comment.
    writeln!(output, "# SPDX-License-Identifier: PMPL-1.0-or-later").expect("TODO: handle error");
    writeln!(
        output,
        "# Auto-generated by julianiser from {}",
        unit.source_path
    )
    .expect("TODO: handle error");
    writeln!(output, "# Source language: {}", unit.language).expect("TODO: handle error");
    writeln!(
        output,
        "# DO NOT EDIT — regenerate with `julianiser generate`"
    )
    .expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");

    // Module declaration.
    writeln!(output, "module {}", unit.module_name).expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");

    // Using statements.
    for pkg in &packages {
        writeln!(output, "using {}", pkg).expect("TODO: handle error");
    }
    ...
    writeln!(output, "export run_pipeline").expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");

    // Generate translated calls.
    writeln!(output, "\"\"\"").expect("TODO: handle error");
    writeln!(output, "    run_pipeline()").expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");
    writeln!(
        output,
        ...
        for call in &unit.detected_calls {
            ...
            match julia_equivalent {
                Some(julia_call) => {
                    ...
                    writeln!(output, "    {}", rendered).expect("TODO: handle error");
                }
                None => {
                    writeln!(
                        output,
    ...
    writeln!(output, "end  # function run_pipeline").expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");
    writeln!(output, "end  # module {}", unit.module_name).expect("TODO: handle error");
...
pub fn generate_project_toml(manifest: &Manifest, units: &[TranslationUnit]) -> String {
    let mut output = String::new();

    writeln!(output, "# SPDX-License-Identifier: PMPL-1.0-or-later").expect("TODO: handle error");
    writeln!(output, "# Auto-generated by julianiser").expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");
    writeln!(output, "name = \"{}\"", manifest.project.name).expect("TODO: handle error");
    writeln!(output, "version = \"{}\"", manifest.project.version).expect("TODO: handle error");
    writeln!(output).expect("TODO: handle error");
    writeln!(output, "[deps]").expect("TODO: handle error");
    ...
    writeln!(output).expect("TODO: handle error");
    writeln!(output, "[compat]").expect("TODO: handle error");
    writeln!(output, "julia = \">= {}\"", manifest.julia.version).expect("TODO: handle error");
